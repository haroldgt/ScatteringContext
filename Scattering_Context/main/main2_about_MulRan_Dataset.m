...
% Demo: Evaluation on MulRan dataset.
clear; clc;
...
Groundtruth_path = 'E:/matrix_labs/Scattering_Context/Groundtruth_pose/mulran/sejong01.mat';
...
for i = 0 : (nodes_num - 1)
    ...
    for ith_candidate = 1 : length(AllCandidates_index)
        ...
        NDD_sim = corr_sim(cat(3,ith_q_scatteringcontext(1:30,:),ith_q_scatteringcontext(31:end,:)), cat(3,ithCandidate_ScanContext(1:30,:),ithCandidate_ScanContext(31:end,:)));
...
title(strcat(' Precision-recall curve of Scattering Context on MulRan',' dataset'));