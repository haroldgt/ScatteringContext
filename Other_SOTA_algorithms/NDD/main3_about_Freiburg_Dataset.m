...
for i = 0 : (nodes_num - 1)
    ...
    ith_q_scancontext = img_ndd(:,61:120);
    NDDContexts{end+1} = ith_q_scancontext;

    ith_q_ringkey = sum(ith_q_scancontext,2);
    ...
    for ith_candidate = 1 : length(AllCandidates_index)
        ...
        NDD_sim = corr_sim(cat(3,ith_q_scancontext(1:15,:),ith_q_scancontext(16:end,:)), cat(3,ithCandidate_ScanContext(1:15,:),ithCandidate_ScanContext(16:end,:)));
...
for loop_thres = 0.02:0.02:1
    ...
    for i = 0 : (nodes_num - 1)
        fprintf('%f, %d\n', loop_thres, i);
        ith_q_scancontext = NDDContexts{i+1};
