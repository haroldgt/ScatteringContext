...
ptcloud_tran =ptcloud.Location;
ptcloud_tran_r = sqrt(ptcloud_tran(:, 1).*ptcloud_tran(:, 1) + ptcloud_tran(:, 2).*ptcloud_tran(:, 2));
outgroup_id = find(ptcloud_tran_r>=max_range);
ptcloud_tran(outgroup_id,:) = [];
