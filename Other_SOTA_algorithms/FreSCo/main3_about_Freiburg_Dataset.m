...
%% { Data Preparation }
LidarData_path = 'G:/big-data/Freiburg/lidar/';
Groundtruth_path = 'E:/matrix_labs/lab0_loopGroundtruth_by_pose/freiburg/freiburg.mat';
...
for i = 0 : (nodes_num - 1)
    ...
    t0 = tic;
    [ith_q_frescontext,ith_q_ringkey] = FreSCo(ith_query_pcd);
    tCount0 = toc(t0);
    Description_consumption = [Description_consumption; tCount0;];
    FreSContexts{end+1} = ith_q_frescontext;

    fresckeys = [fresckeys; ith_q_ringkey'];
    ...
    KDtree_object = createns(fresckeys(1:i-enough_node_diff_num,:),"NSMethod","kdtree"); 
    ...
    for ith_candidate = 1 : length(AllCandidates_index)
        ...
        ithCandidate_ScanContext = FreSContexts{ithCandidate_ID};

        [best_offset, Dist_to_ithCandidate] = computeFrescoDist(ith_q_frescontext, ithCandidate_ScanContext);
...
results = [];
for loop_thres = 5:5:120
    detection_loop = zeros(nodes_num,2);
    
    for i = 0 : (nodes_num - 1)
        fprintf('%f, %d\n', loop_thres, i);
        ith_q_sc = FreSContexts{i+1};
